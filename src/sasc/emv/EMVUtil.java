...
//      //handle procedure bytes here, and not in the lower level TerminalProvider Implementations.
    ...
    public static String getResponse(CardResponse response, boolean doParseTLVData) {
    	StringBuilder dane = new StringBuilder();
        dane.append("response hex    :\n" + Util.prettyPrintHex(response.getData())+"\n");
        String swDescription = "";
        String tmp = SW.getSWDescription(Util.short2Hex(response.getSW()));
        if (tmp != null && tmp.trim().length() > 0) {
            swDescription = " (" + tmp + ")";
        }
        dane.append("response SW1SW2 : " + Util.byte2Hex(response.getSW1()) + " " + Util.byte2Hex(response.getSW2()) + swDescription+"\n");
        dane.append("response ascii  : " + Util.getSafePrintChars(response.getData())+"\n");
        if (doParseTLVData) {
        	dane.append("response parsed :\n" + EMVUtil.prettyPrintAPDUResponse(response.getData())+"\n");
        }
