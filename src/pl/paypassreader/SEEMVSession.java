...
import static pl.paypassreader.Hex.fromHex;
...
public class SEEMVSession {
    ...
    public SmartCard getCurrentCard() {
        return card;
    }

    public SmartCard initCard() throws TerminalException {
        card = new SmartCard(new sasc.iso7816.ATR(seConn.getATR()));
        ...
        if (response.getSW() == SW.SUCCESS.getSW()) {
            card = new SmartCard(new sasc.iso7816.ATR(response.getData()));
        } else {
